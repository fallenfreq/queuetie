...
const shortHand = (check:string) =>
  mimeVals.find(val => val.includes(check))

const parse = (acceptsHeader:string[]) =>
  acceptsHeader.map(val => {
    const sections = val.split(';')
    const accepted = sections[0]
    if (!accepted) throw Error(`Unexpected format of accepts header value "${val}"`)
...
// check for a match
const condition = (accepted:string, type:string) => {
  const full = shortHand(type)
  if (!full) throw Error(type + ' mime type needs to be added')
...
const accepts = (headers:(IncomingMessage & ConfirmedReqProps)['headers']) =>
  (types:string | string[]) => {
    ...
    if (!acceptedTypes || !acceptedTypes[0]) return true
    ...
    for (const { accepted } of accepts) {      
      if (typeof types === 'string' && condition(accepted, types)) {
        ...
        for (const type of types) {
          if (condition(accepted, type)) return type
        }
      }
    }
    return false
