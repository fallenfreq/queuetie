...
import fs from 'fs'
import { mime, type Ext } from './mimeType.js'
...
const cache = createCache<string, Readable>({
  limit: 100000,
  freeUp: 5000
})

// moves on to 404 not found on error
const onStream = (readable:Readable, next:Next) =>
  new Promise((resolve) => {
...
// set headers
const setHeaders = (res:AugmentedResponse, ext:Ext, browserCache:boolean) => {
  if (browserCache) res.setHeader('Cache-Control', 'public, max-age=31536000')
...
const serveStatic = ({
  ...
  return function (req, res, next) {
    ...
    // check for extention
    const ext = path.extname(pathname) as Ext
    if (!ext) return next()
